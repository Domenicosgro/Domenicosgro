...
export default function ActionItems({ items, onChange, agendaItems = [], projectContacts = [], protocolId = null, canManageRelease = false, onOpenBimIssue }) {
  ...
  const toggleDone = (id) => {
    ...
    if (!item) return
    update(id, 'status', item.status === 'erledigt' ? 'offen' : 'erledigt')
  }
  ...
  const visible = regularItems.filter(it => {
    if (q) {
      return (
  ...
  return (
    <div className="card p-6 space-y-4">
      ...
      {visible.length === 0 && regularItems.length > 0 && (
      ...
      {/* BIM-Issues – separater Abschnitt */}
      {bimItems.length > 0 && (
        <div className={regularItems.length > 0 ? 'pt-4 border-t-2 border-dashed border-cyan-200' : ''}>
          <div className="flex items-center gap-2 mb-3">
            <Box size={14} className="text-cyan-600" />
            <span className="text-sm font-semibold text-gray-700">BIM-Issues</span>
            {bimOpen > 0 && (
              <span className="badge text-xs bg-cyan-100 text-cyan-700 border border-cyan-300">{bimOpen} offen</span>
            )}
