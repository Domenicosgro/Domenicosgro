...
export default function ContactAutocomplete({
  ...
  const openMenu = () => { if (contacts.length) { place(); setHi(-1); setOpen(true) } }
  ...
  const onKeyDown = (e) => {
    ...
    else if (e.key === 'Enter')     { if (hi >= 0 && matches[hi]) { e.preventDefault(); choose(matches[hi].label) } }
      ...
      {open && rect && matches.length > 0 && createPortal(
        ...
        >
          {matches.map(({ c, label }, i) => (
            ...
            >
              <span className="text-gray-900 truncate">{c.name || c.company || label}</span>
              <span className="text-xs text-gray-400 truncate">
                {[c.company && c.name ? c.company : null, c.role || c.gewerk, c.email].filter(Boolean).join(' · ')}
              </span>
            </button>
          ))}
        </div>,
